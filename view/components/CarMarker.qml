...
Item {
    ...
    readonly property real arrowCanvasWidth: 46
    readonly property real arrowCanvasHeight: 58
    readonly property real arrowTailY: markerCenterY + arrowVerticalOffset + arrowCanvasHeight / 2 - 5
    readonly property real pulseCenterX: markerCenterX - 14
    readonly property real pulseCenterY: arrowTailY + 4

    Canvas { id: ringsCanvas; anchors.fill: parent
        onPaint: {
            var ctx=getContext("2d"); ctx.clearRect(0,0,width,height)
            var cx=root.pulseCenterX; var cy=root.pulseCenterY
            ...
            ctx.ellipse(cx,cy,34,11,0,0,Math.PI*2); ctx.fill()

            ctx.beginPath(); ctx.strokeStyle="rgba(0,240,255,0.48)"; ctx.lineWidth=1.5
            ctx.ellipse(cx,cy,32,10,0,0,Math.PI*2); ctx.stroke()

            ctx.beginPath(); ctx.strokeStyle="rgba(0,240,255,"+pulseAlpha.toFixed(2)+")"; ctx.lineWidth=1.4
            ctx.ellipse(cx,cy,34*pulseScale,11*pulseScale,0,0,Math.PI*2); ctx.stroke()
        }
        Timer { interval:30; running:true; repeat:true; onTriggered: ringsCanvas.requestPaint() }
    }

    Canvas { id:arrowCanvas; anchors.centerIn:parent; width:root.arrowCanvasWidth; height:root.arrowCanvasHeight; anchors.verticalCenterOffset: root.arrowVerticalOffset
        onPaint: {
            var ctx=getContext("2d"); ctx.clearRect(0,0,width,height)
            // depth shadow
